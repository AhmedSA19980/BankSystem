...
private:
	...
	short _Month = 1;
	short _Year = 1900;
	short _hour = 24; // do not forget to add peroperty set , get  and "=" for 3 new propertyies
	short _Minutes = 60;
	short _second = 60;
...
public:
	...
	clsDate()
	{
		...
		_hour = now->tm_hour + 1; 
		_Minutes = now->tm_min + 1;
		_second = now->tm_sec + 1;
	}
	...
	static string DateToString(clsDate Date, bool FullDateTime = false)
	{
		if(FullDateTime){
		
			return  to_string(Date.Day) + "/" + to_string(Date.Month) + "/" + to_string(Date.Year) + "-" + to_string(Date._hour) + ":" + to_string(Date._Minutes) + ":" + to_string(Date._second);
		}
